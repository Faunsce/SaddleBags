...
namespace anna {
	...
	bool integerCheck(std::string data) {
		...
		for (auto&& slot : data) {
			if (slot == '.') { // Check for decimals, more than one is improper formatting and fails the check
				if (!isFloat) {
					isFloat = true; 
					continue; // Prevent num check from failing on first decimal
				} else {
					return false; // More than one decimal point in number
				}
			}
			if (!(slot >= '0' && slot <= '9')) {
